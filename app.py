...
_SYNTHETIC_HIST_FLAG = MODELS_DIR / ".synthetic_historical"
...
def _generate_synthetic_historical_data() -> pd.DataFrame:
    """
    Generate and save synthetic historical loan loss rate data.

    Used as a fallback when the database (PostgresAgent) is unavailable.
    Statistics match the original 120-month dataset (Jan 2014 – Dec 2023) reported
    in modeling.ipynb:
        Residential_Mortgages : mean=0.746 %, std=0.220 %
        Commercial_Loans      : mean=1.408 %, std=0.235 %
        Credit_Cards          : mean=13.236%, std=6.002 %
        Securities_Portfolio  : mean=0.237 %, std=0.128 %

    Each series is generated with an AR(1) mean-reverting process (seed=42 for
    reproducibility). This is SYNTHETIC data — it is NOT from the original database.
    """
    ...
    try:
        dates = pd.date_range(start="2014-01-31", periods=120, freq="ME")
    except ValueError:
        # pandas < 2.2 uses 'M' for month-end
        dates = pd.date_range(start="2014-01-31", periods=120, freq="M")

    portfolio_stats = {
        1: {"name": "Residential_Mortgages", "mean": 0.746,  "std": 0.220, "phi": 0.80},
        2: {"name": "Commercial_Loans",      "mean": 1.408,  "std": 0.235, "phi": 0.85},
        3: {"name": "Credit_Cards",          "mean": 13.236, "std": 6.002, "phi": 0.90},
        4: {"name": "Securities_Portfolio",  "mean": 0.237,  "std": 0.128, "phi": 0.75},
    }

    rows = []
    for pid, stats in portfolio_stats.items():
        mean, std, phi = stats["mean"], stats["std"], stats["phi"]
        noise_std = std * np.sqrt(1 - phi ** 2)

        y = np.zeros(120)
        y[0] = mean + rng.normal(0, noise_std)
        for t in range(1, 120):
            y[t] = mean + phi * (y[t - 1] - mean) + rng.normal(0, noise_std)
        ...
        for i, date in enumerate(dates):
            rows.append({
                ...
                "portfolio_id": pid,
                "portfolio_name": stats["name"],
                "loss_rate_percent": round(float(y[i]), 4),
            })

    df = pd.DataFrame(rows)
    df.to_csv(hist_path, index=False)
    _SYNTHETIC_HIST_FLAG.touch()  # signal to the UI that synthetic data is active
...
def load_historical_loss_rates():
    ...
    if not hist_path.exists():
        try:
            ...
            # Database unavailable (e.g. PostgresAgent not installed, no .env credentials).
            # Fall back to reproducible synthetic data so the app remains fully functional.
            return _generate_synthetic_historical_data()
...
def generate_real_time_forecast(models, metadata, economic_inputs, n_periods=12):
    ...
    # Determine the last historical date to anchor forecast dates.
    # hist_df['cal_date'].max() returns float NaN (not pd.NaT) when the column
    # is object-dtype with no rows, so we guard explicitly with pd.isna().
    _raw_max = hist_df['cal_date'].max()
    if pd.isna(_raw_max):
        last_hist_date = pd.Timestamp("2023-12-31")
    else:
        last_hist_date = pd.Timestamp(_raw_max)
...
def main():
    ...
    # Notify when historical data was generated synthetically (DB unavailable)
    if _SYNTHETIC_HIST_FLAG.exists():
        st.sidebar.info(
            "**Historical data:** Synthetic (AR(1) process, seed=42)  \n"
            "Real database unavailable — connecting to `ads1.datasci.vt.edu` "
            "and running `modeling.ipynb` would replace this with real data."
        )
    ...
    # =========================================================================
        ...
        economic_inputs['fed_funds_rate'] = st.sidebar.slider(
            "Fed Funds Rate (%)", min_value=0.0, max_value=8.0, value=3.5, step=0.25,
            help="Federal Reserve target rate"
        )

        economic_inputs['vix_level'] = st.sidebar.slider(
            "VIX Level", min_value=10, max_value=85, step=5, value=40,
    ...
    with col2:
        st.metric(label="Capital Adequacy Status", value=status, delta=None)
        if status == "PASS":
    ...
    with col3:
        st.metric(label="Number of Portfolios", value=len(PORTFOLIO_MAP), delta=None)
    ...
    with col1:
        st.plotly_chart(create_bar_chart(scenario_df), use_container_width=True)
    with col2:
        st.plotly_chart(create_forecast_line_chart(scenario_df, hist_df), use_container_width=True)

    col1, col2 = st.columns(2)
    with col1:
        st.plotly_chart(create_capital_gauge(total_loss), use_container_width=True)
    with col2:
